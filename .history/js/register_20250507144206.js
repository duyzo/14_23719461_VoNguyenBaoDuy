    ...
    if (form) {
        ...
        form.password.addEventListener('blur', () => {
            ...
            if (!result.isValid) {
                ...
                clearFieldError(result.field);
                // Kiểm tra lại confirmPassword nếu đã có giá trị
                if (form.confirmPassword.value) {
                    const confirmResult = testPasswordMatch(form.password.value, form.confirmPassword.value);
                    if (!confirmResult.isValid) {
                        showFieldError(confirmResult.field, confirmResult.message);
                    } else {
                        clearFieldError(confirmResult.field);
                    }
                }
            }
        });
